...
public class Logic {
	...
	public Logic() {
		gameBoard = new Board();
		// Next, construct a hashmap String tags -> pieces so that we can
	...
	public void reactToBoardButtonPressed(int x, int y) {
		...
		if (choosePiece) {
			...
			if (gameBoard.boardArray[x][y] == null) {
				// Unoccupied button. Do nothing.
			} else if (gameBoard.boardArray[x][y].tag.charAt(1) == 'P') {
				// Pawn - display move options accordingly.
				currentMoves.addAll(pawnMoveOptions(gameBoard.boardArray[x][y]));
			} else if (gameBoard.boardArray[x][y].tag.charAt(1) == 'T') {
				// Tower - display move options accordingly.
				currentMoves.addAll(towerMoveOptions(gameBoard.boardArray[x][y]));
			} else if (gameBoard.boardArray[x][y].tag.charAt(1) == 'N') {
				// Knight - display move options accordingly.
				currentMoves.addAll(knightMoveOptions(gameBoard.boardArray[x][y]));
			} else if (gameBoard.boardArray[x][y].tag.charAt(1) == 'B') {
				// Bishop - display move options accordingly.
				currentMoves.addAll(bishopMoveOptions(gameBoard.boardArray[x][y]));
			} else if (gameBoard.boardArray[x][y].tag.charAt(1) == 'Q') {
				// Queen - display move options accordingly.
				currentMoves.addAll(queenMoveOptions(gameBoard.boardArray[x][y]));
			} else if (gameBoard.boardArray[x][y].tag.charAt(1) == 'K') {
				// King - display move options accordingly.
				currentMoves.addAll(kingMoveOptions(gameBoard.boardArray[x][y]));
			}
			
			if (gameBoard.boardArray[x][y] != null) currentPiece = gameBoard.boardArray[x][y];
			...
			for (Coord move : currentMoves) {
				if (move.x == x && move.y == y) {
					// Piece is moved to x, y. Check for a checkmate.
	...
	private ArrayList<Coord> knightMoveOptions(Piece knight) {
		...
		// Check all posible "L" moves in any direction. Add to options array as long as cell not occupied by friendly piece.
	...
	private ArrayList<Coord> bishopMoveOptions(Piece bishop) {
		...
		// Check all posible diagonal moves.
	...
	// Check for a checkmate given the current board configurations, and given that the
		...
		for (Coord move : moves) {
			if (p.tag.charAt(0) == 'W') {
				if (move.x == pieceMap.get("BK").x && move.y == pieceMap.get("BK").y) {
					...
					// Implement full checkmate system later.
				}
			} else {
